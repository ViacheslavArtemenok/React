...
import { refreshChats, sendMessageWithThunk, showname } from "./store/profile";
...
export function App({ myName, myMessage }) {
  ...
  const UserAnswer = function () {
    if (userFriend !== "Выберите собеседника React" && value !== "") {
      ...
      if (BotText[countForIndex]) {
        NewProfile.BotAnswer = BotText[countForIndex];
        setcountForIndex((prevcountForIndex) => prevcountForIndex + 1); //На экспорт
      }
      Inpref.current.focus();
  ...
  const ChangeFriend = function (friend, indFriend) {
    setUserFriend(friend); //name чата на экспорт
    setChat(indFriend); //индекс чата в массиве чатов на экспорт
    NewProfile.NewChat = indFriend;
    setcountForIndex(-1);
    Inpref.current.focus();
