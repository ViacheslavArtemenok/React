...
const createMessage = (roomId) => {
  db.ref("messages").child("room1").push({ id: 1, author: "User", message: "" })
}
      ...
      <PersistGate loading={null} persistor={persistore}>
        <BrowserRouter>
          <DefaultThemeProvider themes={themes} initialTheme="light">
            <button onClick={addConversation}>Conv</button>
            <button onClick={createMessage}>CreMess</button>
            <Header isAut={session} />
            <Switch>
              <PrivateRoute
                isAut={session}
                path="/chat"
